...
我们使用 $2 \times 2$ 的  $R$ 控制收敛表现。 在初始选择 $[v_0, x_0]$ $t$步之后变成 $R(\beta, \eta, \lambda)^t [v_0, x_0]$。 因此， 收敛速度取决于 $R$ 的特征值。 $0 < \eta \lambda < 2 + 2\beta$  动量收敛。 这比梯度下降的 $0 < \eta \lambda < 2$ 有更大的可行参数范围。 这也表明通常更大 $\beta$ 更好。 