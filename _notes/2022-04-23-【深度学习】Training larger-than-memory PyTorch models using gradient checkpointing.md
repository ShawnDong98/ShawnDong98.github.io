...
其次，您会注意到我们在模型中的第二个卷积块上使用了 checkpoint，而没有在第一个中使用。这是因为 checkpoint 通过检查输入张量的 `no_grad` 行为，天真地确定其输入函数是否需要梯度下降(例如，是否处于 `no_grad=True` 或 `no_grad=False` 模式)。模型的输入张量几乎总是在 `no_grad=False` 模式，因为我们几乎总是对计算相对于网络权值的梯度感兴趣，而不是输入样本本身的值。因此，在模型的第一个子模块中设置 checkpoint 只会适得其反:它会冻结现有的权重，阻止它们进行训练。