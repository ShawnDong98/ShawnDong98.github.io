...
$$
$m$ 索引 attention head， $k$ 索引采样的 key，并且 $K$ 是总采样的键值数量($K << HW$)。$\Delta p_{mqk}$ 和 $A_{mqk}$ 分别表示第 $m^{th}$ 个 attention head 中的第 $k^{th}$ 个采样点的采样偏移量和注意力权重。scalar attention weight $A_{mqk}$ 的值域在 [0, 1] 之间， 由 $\sum_{k=1}^K A_{meqk} = 1$ 规范化。$\Delta p_{mqk} \in \mathbb{R}^2$ 是无约束范围的 2-d 实数。
