...
**RAG-Sequence** 对于 RAG-Sequence， 似然 $p(y \mid x)$ 没有打破传统 per-token 似然， 因此， 我们不能使用单个 beam search 求解它。 进而，我们为每个文档 $z$ 使用 beam search, 使用 $p_\theta(y_i \mid x, z, y_{1:i-1})$ 打分每个假设。
