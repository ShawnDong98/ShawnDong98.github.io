...
我们使用与[31]相似的骨干，结构如图2所示。主干有两个子网络：一个自上而下的网络以越来越小的空间分辨率产生特征，另一个网络对自上而下的特征进行上采样和拼接。自上而下的主干可以由一系列块 $Block(S，L，F)$ 组成。每个块的步长为 $S$（相对于原始输入伪图像）。块具有带有 $F$ 输出通道的$L$  $3 \times 3$ 2D卷积层，每个通道后跟BatchNorm和ReLU。
