...
在指标张量中指定的索引处，将张量src中的所有值写入self。对于src中的每个值，其输出索引由其在src中的dimension != dim索引和dimension = dim索引中的对应值指定。
...
- input (Tensor) - 输入的张量
- sorted (bool) - 是否在返回作为输出之前按升序对元素进行排序。
- return_inverse (bool) – 是否还返回原始输入中的元素在返回的唯一列表中结束的位置的索引。
- return_counts (bool) – 是否也返回每个元素的计数。
- dim (int) – 应用unique的维度。如果为None，则返回拉平输入的值。默认值:None

输出：

- output (Tensor): 标量元素的输出列表。
- inverse_indices (Tensor):  (可选)如果return inverse为真，则会有一个额外的返回张量(与输入形状相同)，表示原始输入中的元素映射到输出中的索引;否则，这个函数将只返回一个张量。
- counts (Tensor): (可选)如果返回计数为True，将有一个额外的返回张量(与output或output.size(dim)相同的形状，如果指定了dim)，表示每个唯一值或张量出现的次数。
...
- Input：$(N, C, H_{in}, W_{in})$
- Output：$(N, C, H_{out}, W_{out})$， 其中
$$H_{out} = \lfloor \frac{H_{in} + 2 * padding[0]  - dilation[0] \times (kernel\_size[0] -1) -1}{stride[0]} + 1 \rfloor$$

$$W_{out} = \lfloor \frac{W_{in} + 2 * padding[1]  - dilation[1] \times (kernel\_size[1] -1) -1}{stride[1]} + 1\rfloor$$

- dilation： 一个控制在窗口中元素步长的参数
- ceil_mode： 如果为True， 将会使用ceil而不是floor计算输出的形状。
