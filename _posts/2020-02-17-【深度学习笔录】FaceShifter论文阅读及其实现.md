...
这种限制已经被subject-agnostic的人脸交换研究解决了： RSGAN学会分别提取脸部和头发区域向量化的特征， 并重新组合它们来合成交换的人脸。FSNet将源图像的人脸区域表示为一个向量， 这个向量与一个非人脸的目标图像结合生成要交换的人脸。IPGAN将人脸的身份和属性分解为向量通过从源身份和目标图像直接引入监督，IPGAN更好地保留身份特征进行人脸交换。然而，由于压缩表征而引起的信息损失，以及缺乏更多自适应的融合信息， 这三种方法都不能生成高质量的人脸图像。最近，FSGAN还同时进行了面部重现和换脸。虽然FSGAN利用一个识别遮挡的人脸识别网络来保存目标的遮挡，但它很难保留目标的属性，如光照或图像分辨率，它也不能保存源身份的脸型。
