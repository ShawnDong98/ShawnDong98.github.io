...
在针对给定文本序列的手写生成过程中，Graves设计了一种可微的注意力模型，将文本字符与更长的笔迹对齐，这种对齐只能朝一个方向移动[[Graves, 2013]](http://d2l.ai/chapter_references/zreferences.html#graves-2013)。受学习对齐思想的启发，Bahdanau等人提出了一种不受严格单向对齐限制的可微分注意力模型[[Bahdanau等人，2014]](http://d2l.ai/chapter_references/zreferences.html#bahdanau-cho-bengio-2014)。在预测一个 token 时，如果不是所有的输入 tokens 都是相关的，那么模型只会对齐(或参加)与当前预测相关的输入序列部分。这是通过将上下文变量作为注意力池化的输出来实现的。
