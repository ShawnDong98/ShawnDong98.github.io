...
在描述下面关于RNN编码器-解码器的badanau attention时，我们将遵循9.7节中的相同符号。新的基于注意力的模型与第9.7节中相同，只是(9.7.3)中的上下文变量 $c$ 在任何解码时间步 $t'$ 被 $c_{t'}$替换。假设输入序列中有 $T$ tokens，解码时间步 $t$ 处的上下文变量是注意力池化的输出：
