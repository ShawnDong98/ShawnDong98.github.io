...
如图 2 所示， 每个 building block 采用更高分辨率特征图 $N_i$ 和一个更粗粒度的特征图 $P_{i+1}$ 通过水平连接， 然后生成新的特征图 $N_{i+1}$。 每个特征图 $N_i$ 首先通过一个 $3 \times 3$ 卷积层， 步长为2来减小空间大小。然后特征图 $P_{i+1}$ 的每个元素和降采样的特征图通过水平连接相加。
