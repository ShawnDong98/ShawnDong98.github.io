...
在本文中，我们提出了一种基于卷积神经网络工作（CNN）的端到端方法，该方法共同优化了编码孔径和图像重建（图1）。具体来说，对于观测阶段，根据 CASSI 正向模型的性质，我们将 CASSI 正向模型从基于图像的建模分解为基于 patch 的建模，这与CNN的复杂性要求保持一致。这种动机来自一个关键观察，即二维压缩图像的每个小 patch 都对应于潜在 3D HSI 中的 oblique parallelepiped[21]。通过充分利用基于 patch 的建模的优点，我们进一步设计了循环的编码孔径模式，并通过将其视为网络权重来优化编码模板。
