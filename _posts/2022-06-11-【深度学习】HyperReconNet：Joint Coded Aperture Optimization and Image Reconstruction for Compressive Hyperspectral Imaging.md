...
根据CS理论，将 HSI 映射到 CASSI 中压缩图像的感知矩阵在重建质量中起着至关重要的作用。使用固定探测器和色散棱镜，感知矩阵仅由编码孔径的实体决定。编码模板的初始采用随机二值设计。然而，随机编码孔径没有充分利用 CASSI 感知机制的结构，这导致了次优重建。为了使 CASSI 观测包含更多有用的信息，近年来，编码孔径模式的优化引起了越来越多的关注[17]。
