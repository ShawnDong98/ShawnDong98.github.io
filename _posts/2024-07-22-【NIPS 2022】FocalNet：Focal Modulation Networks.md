...
短期和长期上下文对于视觉建模都是重要的。然而，单一的聚合具有更大感受野不仅在时间和内存上计算开销大，而且还削弱了局部细粒度结构，这对于密集预测任务尤其有用。受到[95]的启发，我们提出了一种多尺度分层上下文聚合。正如图4(c)所示，聚合过程由两个步骤组成：分层上下文化以提取不同粒度级别的局部到全局范围的上下文，以及门控聚合以将不同粒度级别的所有上下文特征浓缩到调制器中。
