...
总的来说，这些最新的发展似乎表明，基于分数的生成建模与多个噪声扰动和扩散概率模型是同一模型家族的不同角度，就像波力学和矩阵力学是物理学历史上量子力学的等效公式一样。分数匹配和基于分数的模型的视角允许人们准确地计算对数可能性，自然地解决逆问题，并直接连接到基于能量的模型，Schrödinger桥梁和最优运输。
