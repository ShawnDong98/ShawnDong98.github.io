...
**Comparison with Swin** swin 使用局部窗口注意力， 使用窗口 shift 实现信息交流。这是用来在不同的 patches 之间引入交流，并增加感受野。但是，这个过程相对复杂，并且可能没有在移动设备(如移动设备)上进行速度优化。Swin Transformer依赖于 `torch.roll()` 对特征执行循环移位和它的反过程。这种操作对内存不友好，并且很少被流行的推理框架支持，如NVIDIA TensorRT，谷歌Tensorflow-Lite，和Snapdragon神经处理引擎SDK (SNPE)等。相比之下，Twins模型不需要这样的操作，只涉及在现代深度学习框架中已经优化好的矩阵乘法。
