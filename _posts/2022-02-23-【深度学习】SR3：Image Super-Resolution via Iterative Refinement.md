...
条件 DDPM 模型在 $T$ 个优化步骤生成一个目标图像 $y_0$。 从一个噪声图像 $y_T \thicksim N(0, I)$ 开始， 模型通过学习到的条件转移分布 $p_\theta(y_{t-1} \mid y_t, x)$ ，连续的迭代 ($y_{T-1}, y_{T-2}, ..., y_0$) 迭代优化图像,  最终得到 $y_0 \thicksim p(y \mid x)$。 