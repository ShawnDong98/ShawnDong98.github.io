...
推理链中中间图像的分布是用正向扩散过程定义的，该扩散过程通过固定的马尔可夫链逐渐将高斯噪声添加到信号中，表示为 $q(y_t \mid y_{t-1})$。 我们模型的目标是通过以 $x$ 为条件的反向马尔可夫链迭代地从噪声中恢复信号，从而逆转高斯扩散过程。原则上，每个前向过程步骤也可以以 $x$ 为条件，但我们把它留给未来的工作。我们使用神经去噪模型 $f_\theta$ 学习反向链，该模型将源图像和噪声目标图像作为输入，并估计噪声。