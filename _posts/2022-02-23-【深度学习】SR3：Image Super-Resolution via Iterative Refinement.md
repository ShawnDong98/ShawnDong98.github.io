...
给定一个输出-输出图像对的数据集， 表示为 $D = {x_i, y_i}_{i=1}^N$， 其表示从未知条件分布 $p(y \mid x)$ 中采样。这是一个 one-to-many  的映射， 单个源图像可能对应多个目标图像。我们的目标是通过一个随机迭代优化学习一个参数化估计 $p(y \mid x)$，其将一个源图像 $x$ 映射为一个目标图像 $y \in R^d$。  