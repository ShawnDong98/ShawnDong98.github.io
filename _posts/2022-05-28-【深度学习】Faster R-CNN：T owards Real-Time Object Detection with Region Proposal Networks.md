...
(iii) **Non-approximate joint training.** 如上所述，RPN 预测的边界框也是输入的函数。Fast R-CNN中的RoI 池化层[2]接受卷积特征和预测的边界框作为输入，因此理论上有效的反向传播求解器也应涉及框坐标的梯度。在上述近似联合训练中，这些梯度被忽略了。在非近似联合训练解决方案中，需要一个与边界框坐标可微的 RoI 池化层。
