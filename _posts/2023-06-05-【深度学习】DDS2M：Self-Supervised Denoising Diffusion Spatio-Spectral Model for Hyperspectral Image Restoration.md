...
作为一种新的生成模型，扩散模型[37、13、28、38]因其在图像生成方面的先进性能而吸引了社区的极大关注。从本质上讲，扩散模型是使用变分约束目标训练的参数化采样链，这相当于基于分数的模型[39，40，41]。训练后，样本由采样链生成，从白噪声开始，逐渐去噪到干净的图像。值得注意的是，扩散模型可以超越图像合成[11、32、20]，并已广泛用于图像恢复任务，如超分辨率[16、46、34、6]、inpainting[16、46、33、21、39、41]、去噪[16]等。在这些方法中，基于扩散的图像恢复框架DDRM[16]实现了强大的噪声鲁棒性，这对于高光谱图像（HSIs）来说也是如此。由于有限的光线、光子效应和大气间隙，HSIs 经常遭受噪声损坏[19]。