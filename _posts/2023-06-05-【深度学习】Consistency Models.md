...
为了训练一致性模型，我们提供了两种基于强制执行自一致性属性的方法。第一种方法依赖于使用数值ODE求解器和预训练的扩散模型来生成PF ODE轨迹上的相邻点对。通过最大限度地减少这些对的模型输出之间的差异，我们可以有效地将扩散模型蒸馏成一致性模型，从而允许通过一次网络评估生成高质量的样本。相比之下，我们的第二种方法完全消除了对预训练的扩散模型的需求，允许我们独立地训练一致性模型。