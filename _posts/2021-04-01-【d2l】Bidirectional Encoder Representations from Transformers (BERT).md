...
- word2vec和GloVe等词嵌入模型是与上下文无关的。它们给一个的单词赋值同一个的预训练向量，而不考虑单词的上下文(如果有的话)。它们很难很好地处理自然语言中的一词多义或复杂语义。
