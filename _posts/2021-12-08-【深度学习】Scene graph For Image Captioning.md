...
这篇文章在 Transformer self-attention 层上增加了一个 Probabilistic Graphical Model (PGM)， 它将稀疏假设增加入了原始的全连接注意力。 这样可以避免琐碎的全局依赖关系， 并且可以发现和利用局部上下文。此外，我们将受约束的 self-attention 层堆叠起来， 并在其上增加层次约束， 这样就可以隐式地解析树。