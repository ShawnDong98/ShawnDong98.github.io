...
Kwan等人提出了一种新的目标跟踪和分类方案，使用 pixel-wise coded exposure 相机捕获的压缩观测，并分别使用可见光、短波红外（SWIR）、中波红外（MWIR）和长波红外（LWIR）视频进行了广泛实验，展示了所提方法的有效性[63、65、66]。他们通过合成压缩观测对YOLO进行了重新训练，以提取目标位置，并将它们联系起来以创建轨迹。
