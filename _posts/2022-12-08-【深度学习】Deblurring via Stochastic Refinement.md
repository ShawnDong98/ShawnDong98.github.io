...
第二套工作侧重于使用深度生成模型来解决逆问题。 对于去模糊，生成对抗网络（GAN）已成功应用，并具有竞争力。基于GAN的恢复方法以对抗性损失训练去模糊网络，使恢复的图像在感知上更合理。然而，到目前为止，所提出的方法是确定性的，对抗性损失通常会引入原始干净图像中不存在的伪影，导致大失真（例如超分辨率的[44]）。
# Conclusion