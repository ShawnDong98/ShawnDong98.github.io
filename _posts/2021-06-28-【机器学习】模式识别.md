...
如果在参数 $\theta = \hat theta$ 下 $l(\theta)$ 最大， 则 $\hat theta$ 应是 “最可能” 的参数值， 它是样本集的函数， 记作 $\hat theta = d(x_1, x_2, ..., x_N) = d(\mathcal{X})$。 称作最大似然估计量。
...
- 把 $x$ 的每个分量分成 $k$ 个 等间隔小窗， (若 $x \in E^d$， 则形成 $k^d$ 个小舱)
- 统计落入各个小舱内的样本数 $q_i$
...
$w^T x + b$ 是一个超平面， sign 为符号函数， 因此SVM是一个判别模型。
...
基于 $s$ 和支持向量 $x_1, x_2, ..., x_s$ 的非线性变换（内积）
...
6） 重新计算 $m_i， i=1, ..., c$ 和 $J_c$

7）若连续 $N$ 次迭代 $J_c$ 不改变， 则停止； 否则转 (2)。
