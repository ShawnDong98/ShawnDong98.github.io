...
也就是说 ，一阶估计 $f(x + \epsilon)$ 由 函数值 $f(x)$ 和 在 $x$ 处的一阶导数 $f'(x)$ 得到。假设小的 $\epsilon$ 向负梯度方向移动将减小 $f$ 并非不合理。微课保持简单， 我们选择固定步长大小 $\eta > 0$ 并且选择 $\epsilon = -\eta f'(x)$。 将它插入泰勒展开，我们得到：

$$
f(x - \eta f'(x)) = f(x) - \eta f'^2(x) + O(\eta^2 f'^2(x))
$$

如果导数 $f'(x) \neq 0$ 不会小时， 因为$\eta f'^2(x) > 0$, 我们将得到进展。此外，我们总可以选择足够小的 $\eta$ 使得高阶项变得无关紧要。因此我们得出：
