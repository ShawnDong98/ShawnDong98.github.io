...
应该注意的是，除了即插即用 IR 外，深度展开 IR 还可以结合基于模型的方法和基于学习的方法的优势。它们之间的主要区别在于，后者将截断的展开优化解释为端到端可训练的深度网络，因此通常以更少的迭代产生更好的结果。然而，深度展开的 IR 需要对每项任务进行单独的培训。相反，无需此类额外培训，即插即用IR很容易部署。