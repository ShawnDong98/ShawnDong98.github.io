...
为了保证 $x_k$ 和 $z_k$收敛在一个固定点， 需要一个大的 $\mu$， 其需要一个大的 $K$ 收敛。因此，常见的方式是逐渐增加 $\mu$， 导致一系列 $\mu_1 < ... < \mu_k < ... < \mu_K$ 。 然而，需要引入一个新的参数来控制步长，使参数设置更加复杂。根据（7）， 我们可以滚查到 $\mu$ 控制去噪先验第 $k$ 个迭代的噪声规模 $\sigma_k(\triangleq \sqrt{\lambda / \mu_k})$。 