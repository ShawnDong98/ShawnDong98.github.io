...
**Generating High-Resolution Images** 变压器的注意力机制限制了其输入 $s$ 的序列长度 $h \times w$。当我们在 VQGAN 采用 $m$ 个降采样块来减小图像 $H \times W$ 到 $h = H/w^m \times w = W / 2^m$，我们观察到超过某个关键的 $m$ 值后会带来重构质量的下降， 具体取决于数据集。为了在百万像素状态下生成图像，我们必须按 patch 和裁剪图像，以便在训练期间将 $s$ 的长度限制在最大可行的大小。 为了采样图像，我们以滑动窗口的方式使用 Transformer，如图所示。3.