...
网络必须得到观测到事实的所有结果， 并且存储所有所有信息到节点中作为节点表征。这并不理想， 最好是将问题作为初始化输入， 然后动态的获取所需的事实来回答问题。

我们特别对 端到端的可学习的语义属性的系统， 并且将其应用于需要基于知识和数据推理的问题上 感兴趣。更重要的是， 我们认为这些图神经网络会是迈向结合结构化表征的深度学习模型的重要一步， 其利用了已知结构的优势， 同时可以学习和推理 这些表征。