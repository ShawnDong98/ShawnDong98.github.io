---
layout:     post
title:      "【机器学习】050--判别 or ⽣成：从逻辑回 归说起"
subtitle:   ""
date:       2020-09-26
author:     "ShawnD"
...
线性回归：就是 $w^Tx+b$，得到的结果 $y=w^Tx+b$，它是一个属于 $(-\infty,\infty)$ 的实数
...
![图 1 sigmoid函数示意图](https://images.gitbook.cn/abbcaee0-8de8-11ea-9144-a708da03c9c4)
...
$$
\begin{aligned}
w_{mle} & =argmax_{w}=log~p(Y|X)\\ 
& =argmax_w~log\prod_{i=1}^{N}p(y_i|x_i)\\
& =argmax_w\sum_{i=1}^{N}log~p(y_i|x_i)\\
& =argmax_w\sum_{i=1}^N(y_ilogp_1+(1-y_i)logp_0)\\
& =argmax_w\sum_{i=1}^N(y_ilog\varphi(x;w)+(1-y_i)log(1-\varphi(x;w)))
\end{aligned}
$$
