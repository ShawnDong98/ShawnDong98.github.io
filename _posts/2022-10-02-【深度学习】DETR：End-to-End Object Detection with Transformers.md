...
目标检测的目的是为每个感兴趣的目标预测一组边界框和类别标签。现代检测器通过定义回归和分类问题来间接解决这一集合预测任务，这些问题基于大量的proposals、anchors 或 window centers。它们的性能显著受到后处理步骤的影响，比如消除近似重复的预测、锚点集的设计以及将目标框分配给锚点的启发式方法[51]。为了简化这些流程，我们提出了一种直接的集合预测方法，以绕过这些替代任务。这种端到端的理念已经在复杂的结构化预测任务中取得了显著进展，例如机器翻译或语音识别，但在目标检测中尚未实现：先前的尝试[42,15,4,38]要么添加了其他形式的先验知识，要么在具有挑战性的基准上未能证明其竞争力。本文旨在弥补这一差距。
