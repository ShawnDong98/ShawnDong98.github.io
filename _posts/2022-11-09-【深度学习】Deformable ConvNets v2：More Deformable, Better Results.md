...
为了进一步加强 Deformable ConvNets 操纵空间支持区域的能力，引入了一种调整机制。有了它， Deformable ConvNets 模块不仅可以调整感知输入特征的偏移量，还可以调节来自不同空间位置的输入特征的amplitudes。在极端情况下，模块可以通过将其特征 amplitude 设置为零来决定不感知来自特定位置的信号。
