...
我们采用以图像为中心的训练。 调整图像的大小，使其比例（短边）为800像素。 每个GPU每个小批量有2张图像，每张图像都有 N 个采样的RoI，正负样本比为1:3。对于 C4 主干来说 $N$ 是 64， 对FPN来说， $N$ 是 512。 作者在8个GPU上（所以有效批量大小是16），训练了 160k 迭代，学习率为0.02，在 120k 迭代时减少 10 倍。