...
**RoIAlign:** RoIPool[12]是从每个 RoI 中提取小特征图（例如7×7）的标准操作。RoIPool首先将浮点数 RoI 量化为特征图的离散化，然后将这个量化的RoI细分为 spatial bin，这些 spatial bin本身被量化，最后将每个 bin 覆盖的特征值聚合（通常通过最大池化）。在连续的坐标 $x$ 上计算 $[x / 16]$， 其中 16 是特征图步长， $[·]$ 是取整； 像这样，在划分为 bin 时量化也产生了。