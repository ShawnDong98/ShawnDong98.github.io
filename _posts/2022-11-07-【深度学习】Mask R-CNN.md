...
**Mask R-CNN:** Mask R-CNN采用相同的两阶段方式，具有相同的第一阶段（即RPN）。在第二阶段，在预测类和框偏移的同时，Mask R-CNN 还为每个 RoI 输出一个二分类 mask。相比于最近的系统， 他们的分类依赖于 mask 预测。所提方法遵循 Fast R-CNN[12]的精神，该精神在并行中应用边界框分类和回归（事实证明，这在很大程度上简化了原始 R-CNN 的多阶段 pipeline[13]）。
