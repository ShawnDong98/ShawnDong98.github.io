...
分别把训练样本和生成器生成的样本送到分类器中（例如Inception Net-V3或者其他CNN等）， 抽取分类器的中间层的抽象特征，并假设该抽象特征符合多元高斯分布， 估计生成样本高斯分布的均值$\mu_g$和方差$\sigma_g$， 以及训练样本$\mu_{data}$和方差$\sigma_{data}$， 计算两个高斯分布的弗雷歇距离， 此距离即FID：

$$\|\mu_{data} - \mu_g\| + tr(\sigma_{data} + \sigma_g - 2(\sigma_{data}\sigma_g)^{\frac{1}{2}})$$ 
