...
为了解决这些挑战，我们提出了一种新的 super token interaction network (SPIN)（SPIN）。我们的方法采用超像素对局部相似像素进行聚类，形成可解释的局部区域，并利用超像素内部注意力实现局部信息交互。它是可解释的，因为只有相似的区域相辅相成，不同的区域被排除在外。此外，我们设计了一种超像素交叉注意力模块，通过超像素的代理来促进信息传播。

广泛的实验表明，所提出的SPIN模型在准确性和轻量级方面与最先进的超分辨率方法相比表现出色。