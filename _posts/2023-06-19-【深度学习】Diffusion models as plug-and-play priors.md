...
即插即用先验的一个明显应用是条件图像生成。 例如，在MNIST数字图像上训练的去噪扩散模型可能会定义 $p(x)$ ，而约束 $c(x，y)$ 可能是现成分类器下数字类 $y$ 的概率。然而，改变 $x$ 的语义，我们也可以将此类模型用于神经网络与域适应的推理任务，例如图像分割： $c(x，y)$ 约束分割 $x$ 以匹配外观或弱标记 $y$。