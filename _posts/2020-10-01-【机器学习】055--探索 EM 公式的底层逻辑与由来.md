...
$$
\begin{aligned}
log~p(x|\theta) & =\int_{z}q(z)log\frac{p(x,z|\theta)}{q(z)}dz-\int_{z}q(z)log\frac{p(z|x,\theta)}{q(z)}dz\\
& =\int_{z}q(z)log~\frac{p(x,z|\theta)}{q(z)}dz+\int_{z}log~q(z)\frac{q(z)}{p(z|x,\theta)}dz
\end{aligned}
$$
...
$$
\begin{aligned}
\theta^{(t+1)}_{mle} & = argmax~log~p(x|\theta)\\
& =argmax_{\theta}[\int_{z}q(z)log~\frac{p(x,z|\theta)}{q(z)}dz+\int_{z}log~q(z)\frac{q(z)}{p(z|x,\theta^{(t)})}dz]
\end{aligned}
$$
...
\begin{aligned}
\theta^{(t+1)}_{mle} &= argmax~log~p(x|\theta)\\
& =argmax_{\theta}[\int_{z}p(z|x,\theta^{(t)})log~\frac{p(x,z|\theta)}{p(z|x,\theta^{(t)})}dz+0]\\
