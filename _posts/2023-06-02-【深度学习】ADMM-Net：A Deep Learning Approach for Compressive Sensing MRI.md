...
\end{cases} \tag{5}
$$
其中 $n \in {1, 2, ..., N_s}$ 表示第 n 个迭代并且上标 T 分别指实数和复数的转置和共轭转置运算符。操作 $x^{(n)}$ 可以通过快速傅里叶变换高效地计算，因为 $P^T P + \sum_{l=1}^L \rho_l F D_l^T D_l F^T$ 是一个对角矩阵。 $S(·)$ 是一个参数为 $\lambda_l / \rho_l, l \in {1, 2, ..., L}$ 的非线性 shrinkage 函数。 它通常是一个软或硬的阈值函数，分别对应于 $l_1$ 和 $l_0$ 正则化器的稀疏正则化。 
