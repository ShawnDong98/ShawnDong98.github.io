...
为了生成3D目标的标签，我们聘请了一组标注员，并要求他们以3D边界框的形式为汽车、面包车、卡车、电车、行人和非机动车等物体分配 tracks。与大多数现有的标记不同，我们不依赖在线众包来执行标注。为了实现这一目标，作者创建了一个特殊的目的标注工具，该工具显示3D激光点以及相机图像，以提高标注的质量。与[16]类似，我们要求标注员将每个边界框附加标记为可见、半遮挡、完全遮挡或截断。
