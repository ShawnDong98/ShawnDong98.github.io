...
我们可以通过将预测序列与标签序列(ground-truth)进行比较来评估该序列。BLEU (Bilingual Evaluation Understudy)虽然最开始提出用于评价机器翻译的结果， 现在已经被大量地用于评估不同应用输出序列的质量。原则上， 对于所有预测序列的 n-grams(n-grams 表示连续的n个词)， BLEU 评估该 n-grams 是否出现在标签序列中。 

用 $p_n$ 表示 n-grams 的准确度， $pn$ 表示预测序列与标签序列中匹配的 n-grams 的数量 与 预测序列中 n-grams 数量 的 比值。给定一个标签序列 A, B, C, D, E, F 和一个预测序列 A, B, B, C, D， 我们有 $p_1 = 4 /5, p_2 = 3/4, p_3 = 1/3, p_4 = 0$。 此外， 用 len~label~ 和 len~pred~ 分别表示标签序列和预测序列的词的数量。BLEU定义如下：

$$
exp(\min (0, 1 - \frac{len_{label}}{len_{pred}})) \prod_{n=1}^k p_n^{1/2^n}
$$

其中 $k$ 是匹配的最长的 n -grams. 
...
||BLEU-1|BLEU-4|METOR|Rough|CIDEr|SPICE|
|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
||66.4|24.5|22.8|49.4|78.3|15.3|
