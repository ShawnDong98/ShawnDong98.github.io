...
用 $p_n$ 表示 n-grams 的准确度， $pn$ 表示预测序列与标签序列中匹配的 n-grams 的数量 与 预测序列中 n-grams 数量 的 比值。给定一个标签序列 A, B, C, D, E, F 和一个预测序列 A, B, B, C, D， 我们有 $p_1 = 4 /5, p_2 = 3/4, p_3 = 1/3, p_4 = 0$。 此外， 用 len~label~ 和 len~pred~ 分别表示标签序列和预测序列的词的数量。
