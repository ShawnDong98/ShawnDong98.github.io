...
我们可以通过将预测序列与标签序列(ground-truth)进行比较来评估该序列。BLEU (Bilingual Evaluation Understudy)虽然最开始提出用于评价机器翻译的结果， 现在已经被大量地用于评估不同应用输出序列的质量。原则上， 对于所有预测序列的 n-grams， BLEU 评估这个 n-grams 是否出现在标签序列中。 
