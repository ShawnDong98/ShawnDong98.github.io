...
一层LSTM中有hidden\_size个LSTM单元， 每个单词都会进入这些单元， 也就是说每个单词都会通过这些LSTM， 也就是所有单词共用这些LSTM单元， 所以说是共享权重， 所以能画成一个细胞不断循环的表现形式。

一层LSTM中的多个LSTM单元， 各个LSTM单元并不会把自身的输出h和C传给其他LSTM单元， 而是将它们的输出组成一个向量，因此一层的输出维度是h\_t.size() = c\_t.size() = (1, batch, hidden_size)。

LSTM中hidden\_state既作为当前时间步的隐藏状态，又作为当前时间步的输出，  c仅起记忆细胞保存信息的作用， 因此输出的最后一个时间步，也就是最后一个词的输出， 就等于最后一个时间步输出的隐藏状态。
