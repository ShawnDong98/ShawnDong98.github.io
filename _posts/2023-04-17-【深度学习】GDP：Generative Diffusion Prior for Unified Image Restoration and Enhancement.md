...
在这项研究中，我们的目标是利用训练好的 DDPM 作为统一图像恢复和增强的有效先验，特别是处理各种类型的退化图像。 假设退化图像 $y$ 通过 $y = D(x)$ 捕获， 其中 $x$ 是原始的自然图像， $D$ 是退化模型。我们使用存储在一些先验中的 $x$ 的统计信息，并在 $x$ 的空间中搜索与 $y$ 最匹配的最优 $x$，将 $y$ 视为 $x$ 的退化观测。由于本文表1中 GAN-inversion 性能有限，以及之前作品[32、33、62、69]的应用受到限制，我们专注于研究一种更通用的图像，即在用于图像合成的大规模自然图像上训练的扩散模型。
