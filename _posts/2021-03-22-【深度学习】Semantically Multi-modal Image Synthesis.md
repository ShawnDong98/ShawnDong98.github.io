...
**Overview.** 如图2所示， GroupDNet由一个编码器和一个解码器组成。 受到 VAE 和 SPADE 的启发， 编码器 $E$ 产生一个隐编码 $Z$， 它在训练期间服从高斯分布 $N(0, 1)$。 在测试期间， 编码器 $E$ 弃用，  $ Z$ 从高斯分布随机采样编码。 为了实现这一目标，我们使用重新参数技巧\[24\]在训练过程中使得损失函数可微。具体地，编码器通过两个全连接层预测均值矢量和方差矢量以表示编码的分布。编码器分布和高斯分布可以通过KL散度损失最小化：
