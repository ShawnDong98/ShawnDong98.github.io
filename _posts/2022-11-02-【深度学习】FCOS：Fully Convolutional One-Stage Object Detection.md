...
根据FPN[14]，作者在不同层级的特征图上检测到不同大小的目标。具体来说，作者使用五级特征图，定义为 ${P_3, P_4, P_5, P_6, P_7}$。 $P_3, P_4$ 和 $P_5$ 由主干 CNN 的特征图 $C_3, C_4, C_5$ 后接一个带有自顶向下的连接的 $1 \times 1$卷积层得到， 如图2所示。$P_6$和 $P_7$ 通过在 $P_5$ 和 $P_6$ 上分别使用步长为2的一个卷积层得到。
