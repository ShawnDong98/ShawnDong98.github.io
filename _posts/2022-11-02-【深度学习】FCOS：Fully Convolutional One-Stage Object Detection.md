...
在这里，作者展示了如何通过 FPN 的多级预测来解决所提的FCOS的两个可能问题。1）CNN中最终特征图的大步长（例如16×）可能导致相对较低的最大可能召回（BPR）。对于基于先验框的检测器，由于大步长而导致的低召回率可以通过降低正先验框所需的 IoU 分数来补偿。对于FCOS，乍一看，人们可能会认为BPR可能比基于先验框的检测器低得多，因为由于大步长，无法召回最终特征图上没有编码的物体。
