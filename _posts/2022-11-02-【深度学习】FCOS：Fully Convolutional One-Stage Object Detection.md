...
对于在特征图 $F_i$ 上每个 $(x, y)$ 位置， 我们可以将其映射回输入图像 $(\lfloor \frac{s}{2} \rfloor + xs, \lfloor \frac{s}{2} \rfloor + ys )$， 其离位置 $(x, y)$ 的感受野中心近。  与基于先验框的检测器不同，基于先验框的检测器将输入图像上的位置作为（多个）先验框的中心，并以这些先验框作为参考回归目标边界框，我们直接在该位置回归目标边界框。
