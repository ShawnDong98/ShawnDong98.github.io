...
为了产生层次化的表征， tokens的数量会随着网络的加深通过 patch merging 减少。 第一个 patch merging 层拼接每个相邻 $2 \times 2$的 patches的特征， 然后对拼接的 $4C$ 维度使用一个线性层。 这将以 $2 \times 2 = 4$ 的速度减少 tokens 的数量， 输出维度被设置为 $2C$。
