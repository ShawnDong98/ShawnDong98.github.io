...
**Swin Transformer block**  Swin Transformer 将 multi-head self attention(MSA) 模块替换为一个基于 基于滑动窗口的模块， 其他结构保持不变。  Swin Transformer block 由一个基于MSA的滑动窗口， 接着一个两层的 MLP 和在它们之间的GELU 非线性激活组成。 LayerNorm(LN) 用于每个 MSA 和 MLP 之间， 并且在每个模块之后加入残差连接。
