...
对于高斯扩散和二项式扩散，对于连续扩散(限制小步长$\beta$)，扩散过程的逆过程具有与正向过程相同的函数形式。因为 $q(x^{(t) \mid x^{(t-1)}})$ 是一个小的高斯(二项)分布，如果 $\beta_t$ 很小， 那么 $q(x^{(t-1) \mid x^{(t)}})$ 将也是一个高斯(二项)分布。扩散的步数越多， 扩散率 $\beta$ 可以做的越小。
