...
**Depth estimation.** 我们已经表明，增加的深度往往会使输出过于平滑，对细粒度图像的影响远远大于粗粒度图像。对于细粒度图像，我们有三种选择：a）向深层网络添加更多跳过连接；b）只需使用较浅的连接；c）保留所有层，但减少向下/向上采样层。我们建议c用于仅解码器的架构，因为它们已经参数化不足；对于 U 型网络，这很容易导致过度拟合（图6）。