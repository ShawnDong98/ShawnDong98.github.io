...
多样化且几乎无限的文本的一个有前景的来源是网络抓取，例如 Common Crawl。虽然这些文件比当前的语言建模数据集大很多数量级，但它们存在重大的数据质量问题。Trinh & Le (2018) 在他们关于常识推理的工作中使用了爬虫，但指出大量文档 “其内容大部分是不可理解的”。我们在 Common Crawl Trinh & Le (2018) 的初始实验中观察到了类似的数据问题，使用一个小的 Common Crawl 子样本获得了最佳结果，该子样本仅包含与其目标数据集最相似的文档，即 Winograd 模式挑战。 虽然这是提高特定任务性能的实用方法，但我们希望避免提前对要执行的任务做出假设。


进而，我们创建了一个新的web scrape，它强调文档质量。为了做到这一点，我们只需要抓取已经由人类策划/过滤的网页。手动过滤一个完整的网络抓取会非常昂贵，所以作为一个起点，我们从 Reddit 抓取了所有出站链接，Reddit 是一个社交媒体平台。这可以被认为是一个启发性的指标，其他用户是否觉得链接有趣，教育，或只是有趣。

生成的数据集WebText包含这4500万个链接的文本子集。 为了从HTML响应中提取文本，我们结合使用Dragnet（Peters&amp;Lecocq，2013）和 Newspaper content extractors。本文中的所有结果都使用了WebText的初步版本，该版本不包括2017年12月之后创建的链接，并且在重复数据消除和一些基于启发式的清理之后，包含了800多万个文档，总共 40 GB的文本。
