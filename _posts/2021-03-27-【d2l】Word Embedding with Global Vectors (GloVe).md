...
然而，交叉熵损失函数尽管是最常见的损失函数类型，但它有时不是一个好的选择。一方面， 让模型预测 $q_{ij}$ 的成本变成合法的概率分布， 需要让它的分母是整个词典的所有项之和。另一方面，字典中往往有很多不常见的词，它们在数据集中很少出现。在交叉熵损失函数中，对大量很少出现的词的条件概率分布的最终预测很可能是不准确的。
