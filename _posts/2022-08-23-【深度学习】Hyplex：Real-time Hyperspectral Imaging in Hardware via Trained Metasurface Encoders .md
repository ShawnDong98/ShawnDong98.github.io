...
在主要数据集中，作者提供了真实和人造水果的高光谱图像。杂项类对应于没有天然对应物的水果和蔬菜。大约40%的场景由位于相机焦平面上的一排物体组成。剩余的场景显示了两排物体，焦平面位于两者之间。作者在整个数据集中保持白色参考面板的位置大致恒定，以便于规范化。高光谱图像的空间分辨率为512×512像素和204个光谱带。作者还为具有相同空间分辨率的每个场景提供通过相机镜头看到的RGB图像。为了验证框架的泛化能力，作者用20张额外的野外图像增强了数据集（示例见图3）。