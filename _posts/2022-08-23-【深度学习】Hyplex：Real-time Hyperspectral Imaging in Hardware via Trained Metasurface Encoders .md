...
为了训练和验证HyplexTM系统，作者使用三个公开的数据集：CAVE数据集，由32张涵盖 400nm 至 700 nm的室内图像组成，以及 Harvard 和 KAUST 数据集，其中包含室内和室外场景，规格为75张和409张图像，光谱带分别覆盖420 nm 至 720nm 和 400 nm 至 700 nm。作者创建一个额外的透明数据集FVgNET。FVgNET由317个场景组成，展示了天然和手工水果和蔬菜，在受控照明条件下在室内拍摄，覆盖 400nm 至 1000 nm 的范围。我们使用由排列成无穷大曲线的白皮纸组成的设置来获得图像，这种配置在摄影中用于将物体与背景隔离开来。
