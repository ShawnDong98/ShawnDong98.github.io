...
在这种方法中， 作者将解码操作 $\mathcal{D}$ 表示为一组层次化非线性操作 $\mathcal{F}$， 其将输入张量 $\hat S$ 投影为输出观测张量 $\hat y$。这个过程通过监督学习迭代训练， 将 $\hat y$ 与真实张量 $\tilde y$ 比较。这种端到端训练找到最优特征空间 $\hat S$ 和相关的线性映射 $\Lambda$。 
