...
深卷积架构，如[11，17]构建对各种任务有效的图像表征。这些架构被用于检测，尽管主要是通过将它们调整为分类或回归框架。深度表征有足够的能力共同编码多个实例的外观，但必须用多个实例预测的组件来增强它们，以实现这一潜力。在本文中，作者考虑了循环神经网络（RNN），特别是 LSTM 单元作为此类组件的候选。
