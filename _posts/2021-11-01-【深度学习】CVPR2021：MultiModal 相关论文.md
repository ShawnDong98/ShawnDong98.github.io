...
基于 OCR 的 image captioning 目标是基于图像中 visual entities（同时包括 visual objects 和 scene text）自动描述图像。与传统的 image captioning 相比，基于 OCR 的 image captioning 需要场景文本的推理，因为生成的描述往往包含多个OCR token。