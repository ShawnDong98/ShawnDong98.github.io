...
受这一无监督工作的启发，作者引入了一种名为去噪扩散恢复模型（DDRM）的有效方法，可以在低至20个NFE中实现竞争结果。DDRM是一个去噪扩散生成模型，它以测量和逆问题为条件，逐渐随机地将样本去噪为所需的输出。通过这种方式，作者引入了一个变分推理目标，用于学习逆问题的后验分布。然后，我们展示了它与无条件去噪扩散生成模型目标的等价性，这使作者能够在DDRM中为各种线性逆问题部署此类模型。