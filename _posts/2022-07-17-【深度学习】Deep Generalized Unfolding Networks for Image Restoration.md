...
考虑到大多数DUN方法的  intrinsic information 损失，设计了每个scale 的 inter-stage information pathways，以在不同 stage 从编码器和解码器广播有用的信息。在图2中使用不同颜色的线来分辨不同 scale 的编码器和解码器信息。为了融合 inter-stage 信息，我们在编码器的每个刻度上设计了一个阶段间特征融合子模块（ISFF）。
