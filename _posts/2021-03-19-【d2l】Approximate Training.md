...
然而，模型中包含的事件只考虑正面的例子。在这种情况下，只有当所有词向量都相等且值趋近于无穷时，上述联合概率才能最大化为1。显然，这样的词向量是没有意义的。负采样通过对目标函数进行正采样和负抽样，使目标函数更有意义。假设 当上下文词 $w_o$ 出现在 中心目标词 $w_c$ 的上下文窗口时， 事件 $P$ 发生， 并且我们根据分布 $P(W)$ 采样 $K$ 个没有出现在上下文的词， 将其视作噪声词。我们假设噪声词 $w_k(k=1, ..., K)$ 不出现在中心目标词 $w_c$的上下文窗口的事件为 $N_k$。 假设事件 $P$ 和 $N_1, ..., N_k$ 对于正样本和负样本是相互独立的。通过负采样， 我们可以重写上述只考虑正样本的联合概率
...
$$P(w_o \mid w_c) = \prod_{j=1}^{L(w_o) - 1} \sigma([[n(w_0, j+1) = leftChild(n(w_o, j))]] · u^T_{n(w_o, j)} v_c)$$
