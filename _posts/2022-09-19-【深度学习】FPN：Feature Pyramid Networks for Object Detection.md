...
我们根据先验框的交并比（IoU）为先验框分配训练标签。 形式上，如果先验框与真实框接的IoU最高，或者与任何真实框的 IoU 超过0.7，则先验框将被分配一个正标签，如果所有真实框的 IoU 小于0.3，则先验框将被分配负标签。请注意，真实框没有被明确用于将它们分配给金字塔的层级；相反，真实框与先验框相关联，其已经被分配给了金字塔层级。
