...
作者通过用FPN取代单尺度特征图来适应RPN。作者在特征金字塔的每个层级上都附加一个相同设计的 head（$3 \times 3$ conv和两个 $1 \times 1$ convs）。因为 head 在所有金字塔层级的所有位置上密集滑动，在特定的层级上没有必要有多尺度的先验框。相反，作者为每个层级分配一个单一尺度的先验框。
