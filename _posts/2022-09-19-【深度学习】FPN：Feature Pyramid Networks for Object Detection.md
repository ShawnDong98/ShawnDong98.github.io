...
**Bottom-up pathway.** 自下而上的路径是主干ConvNet的前向计算，它提出了一个由几个尺度特征图组成的特征层次结构，缩放步长为2。通常有很多层产生相同大小的输出映射，我们说这些层处于相同的网络阶段。对于特征金字塔，作者为每个阶段定义一个金字塔级别。作者选择每个阶段最后一层的输出作为参考特征图集，作者将丰富它来创建金字塔。
