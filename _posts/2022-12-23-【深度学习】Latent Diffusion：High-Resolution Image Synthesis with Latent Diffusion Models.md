...
在图7中，我们将在 CelebA HQ 和 ImageNet 上训练的模型与 DDIM 采样器比较了不同数量的去噪步骤的采样速度，并根据FID分数进行绘制。LDM-{4-8}的表现优于感知和压缩比不恰当的模型。特别是与基于像素的LDM-1相比，它们的FID分数要低得多，同时显著提高了样本吞吐量。ImageNet等复杂数据集需要降低压缩，以避免降低质量。
