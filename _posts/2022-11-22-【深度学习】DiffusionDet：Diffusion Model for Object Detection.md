...
在这项工作中，作者提出了一种新的检测范式 DiffusionDet，将目标检测视为从噪声边界框到目标边界框的去噪扩散过程。

作者的噪声到边界框流程具有几种吸引人的特性，包括动态边界框和渐进细化，使作者能够使用相同的网络参数来获得所需的速度精度权衡，而无需重新训练模型。

标准检测基准的性能表明，与成熟的检测器相比，DiffusionDet 实现了良好的性能。