...
在这项研究中，我们为HSI提出了一种无参考质量评估方法。HSI具有一些对失真敏感的统计特性，这些统计数据与常规统计数据的偏差反映了失真程度。这些统计数据可以作为质量敏感特征提取。通过分析原始和失真的HSI的统计属性，我们提取了光谱和空间领域的多个质量敏感特征。在整合所有这些特征后，我们可以从原始的高光谱训练数据集中学习这些特征的多元高斯模型（MVG）。学习的MVG被视为与在重建的HSI上的MVG模型进行比较的基准。
