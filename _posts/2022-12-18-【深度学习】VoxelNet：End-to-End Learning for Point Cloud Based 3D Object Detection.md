...
**Car Detection** 对于这项任务，我们分别考虑了沿着 Z、Y、X轴 [−3, 1] × [−40, 40] × [0, 70.4]米范围内的点云。投影在图像边界之外的点将被删除[5]。我们选择一个体素大小 $v_D = 0.4, v_H  0.2, v_W = 0.2$ 米， 其导致 $D' = 10, H' = 400, W' = 352$。 我们将 $T = 35$ 设置为每个非空体素中随机采样点的最大数量。
