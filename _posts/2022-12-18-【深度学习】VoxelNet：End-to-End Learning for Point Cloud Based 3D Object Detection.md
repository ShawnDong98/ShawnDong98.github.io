...
**Evaluation in 3D** 与鸟瞰图检测（只需要准确定位二维平面上的物体）相比，3D检测是一项更具挑战性的任务，因为它需要在3D空间中对形状进行更精细的定位。表2总结了比较。对于Car类，VoxelNet在所有难度级别上的表现都明显优于所有其他方法。具体来说，仅使用激光雷达，VoxelNet在简单， 中等和困难级别上分别优于基于激光雷达+RGB的最新方法MV（BV+FV+RGB），分别优于10.68%、2.78%和6.29%。
