...
**Sparse Tensor Representation** 通过仅处理非空体素，我们得到了一个体素特征列表，每个特征都与特定非空体素的空间坐标唯一相关。获得的体素级特征列表可以表示为稀疏的4D张量，大小为 $C \times D′  \times H′ \times W′$ 如图2所示。虽然点云包含约10万个点，但超过90%的体素通常是空的。
