...
对于每个类，检测结果根据三个难度级别进行评估：简单、中等和困难，这些难度级别根据目标大小、遮挡状态和截断水平确定。由于测试集的标签不可用，并且对测试服务器的访问有限，我们使用[4、3、5]中描述的协议进行全面评估，并将训练数据细分为训练集和验证集，从而生成3,712个用于训练的数据样本和3,769个数据样本进行验证。
# Conclusion
