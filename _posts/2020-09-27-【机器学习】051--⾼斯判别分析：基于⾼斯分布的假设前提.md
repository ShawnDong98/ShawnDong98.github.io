---
layout:     post
title:      "【机器学习】051--高斯判别分析：基于高斯分布的假设前提"
subtitle:   ""
date:       2020-09-27
author:     "ShawnD"
...
$$
\begin{aligned}
L(\theta) &= log\prod_{i=1}^N(p(x_i \mid y_i)p(y_i))\\
& =\sum_{i=1}^Nlog(p(x_i \mid y_i)p(y_i))\\
...
$$\sum_{i=1}^N(y_i-\phi)=\sum_{i=1}^Ny_i-\sum_{i=1}^N\phi=\sum_{i=1}^Ny_i-N\phi=0$$
...
$$
\begin{aligned}
\mu_1 &= argmax_{\mu_1}\sum_{i=1}^Nlog~N(\mu_1,\Sigma)^{y_i}\\
& =argmax_{\mu_1}\sum_{i=1}^Ny_ilog\frac{1}{(2\pi)^{\frac{p}{2}} \mid \Sigma \mid ^{1/2}}exp{-\frac{1}{2}(x_i-\mu_1)^T\Sigma^{-1}(x_i-\mu_1)\\}\\
& =argmax_{\mu_1}\sum_{i=1}^Ny_ilog~exp{-\frac{1}{2}(x_i-\mu_1)^T\Sigma^{-1}(x_i-\mu_1)\\}\\
& =argmax_{\mu_1}\sum_{i=1}^Ny_i(-\frac{1}{2}(x_i-\mu_1)^T\Sigma^{-1}(x_i-\mu_1))
\end{aligned}
$$
...
$$\frac{\partial}{\partial
\mu_1}-\frac{1}{2}\sum_{i=1}^Ny_i(-2\mu_1^T\Sigma^{-1}x_i+\mu_1^T\Sigma^{-1}\mu_1)=-\frac{1}{2}\sum_{i=1}^Ny_i(-2\Sigma^{-1}x_i+2\Sigma^{-1}\mu_1)=0$$

$$\sum_{i=1}^Ny_i(\mu_1-x_i)=0\Rightarrow
...
$$\sum_{i=1}^Nlog N(\mu_1,\Sigma)^{y_i}=\sum_{i=1}^Ny_ilog~N(\mu_1,\Sigma)=\sum_{x_i
...
$$
\begin{aligned}
\sum_{i=1}^Nlog~N(\mu,\Sigma) &=\sum_{i=1}^Nlog\frac{1}{(2\pi)^{\frac{p}{2}} \mid \Sigma \mid ^\frac{1}{2}}e^{\frac{1}{2}(x-\mu)^T\Sigma^{-1}(x-\mu)}\\
& =-\sum_{i=1}^N\frac{p}{2}log(2\pi)-\sum_{i=1}^N\frac{1}{2}log \mid \Sigma \mid -\sum_{i=1}^N\frac{1}{2}(x-\mu)^T\Sigma^{-1}(x-\mu)
\end{aligned}
$$
...
$$
\begin{aligned}
\sum_{i=1}^Nlog~N(\mu,\Sigma) & = -\sum_{i=1}^N\frac{p}{2}log(2\pi)-\sum_{i=1}^N\frac{1}{2}log \mid \Sigma \mid -\sum_{i=1}^N\frac{1}{2}(x-\mu)^T\Sigma^{-1}(x-\mu)\\
& =C-\frac{1}{2}Nlog \mid \Sigma \mid -Ntr(S\Sigma^{-1})
\end{aligned}
$$
...
& =-\frac{1}{2}N_1log \mid \Sigma \mid -\frac{1}{2}N_1tr(S_1\Sigma^{-1})-\frac{1}{2}N_2log \mid \Sigma \mid -\frac{1}{2}N_2tr(S_2\Sigma^{-1})+C\\
& =-\frac{1}{2}Nlog \mid \Sigma \mid -\frac{1}{2}N_1tr(S_1\Sigma^{-1})-\frac{1}{2}N_2tr(S_2\Sigma^{-1})+C
\end{aligned}
$$
...
$$\frac{\partial tr(AB)}{\partial A}=B^T$$$$\frac{\partial  \mid A \mid }{\partial
A}= \mid A \mid A^{-1}$$$$tr(AB)=tr(BA)$$$$tr(ABC)=tr(CAB)=tr(BCA)$$

好了，有了这几个公式作为利器，似然函数的求导问题就迎刃而解了：

$$\frac{\partial}{\partial\Sigma}[Nlog \mid \Sigma \mid +N_1tr(S_1\Sigma^{-1})+N_2tr(S_2\Sigma^{-1})]=N\frac{1}{ \mid \Sigma \mid } \mid \Sigma \mid \Sigma^{-1}-N_1S_1\Sigma^{-2}-N_2S_2\Sigma^{-2}=0$$
