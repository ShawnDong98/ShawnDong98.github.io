...
Faster R-CNN 是一种两阶段的目标检测网络。 第一个阶段叫做  Region Proposal Network (RPN)， 提议出可能存在目标的区域。 在每个空间位置网络预测一个 class-agnostic 目标分数 和 多个尺度和比例的 anchor 框的 bbox refinement。 使用非极大值抑制(NMS)通过交并比(IoU)阈值过滤， 将结果作为第二阶段的输入。


Faster R-CNN 可以视作一种有效的 “硬” 注意力机制。
