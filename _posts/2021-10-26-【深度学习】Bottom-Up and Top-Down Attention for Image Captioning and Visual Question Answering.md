...
在这篇文章中， 使用 ResNet-101 CNN 作为主干的 Faster R-CNN。 为了得到用于 Image Captioning 和 VQA 的图像特征 $V$， 将每个类别通过非极大值抑制使用 IoU 阈值过滤， 然后选择所有超过置信度阈值的区域。 对于每个选择出来的区域 $i$， $v_i$ 定义为该区域经过平均池化得到的卷积特征， 其图像特征的维度为 2048。 
