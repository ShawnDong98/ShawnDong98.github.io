...
**He et al.** 是最早的单一图像去雾技术之一。他们引入了暗通道先验（DCP），这是暗对象的扩展。许多最近的图像测深技术都使用DCP来估计光学模型的传输图。该统计数据基于以下观察，即大多数户外图像区域的像素在至少一种颜色通道中呈现低强度。这一规则的例外表现为天空和雾霾区域。DCP有助于粗略估计（每个patch）传输图（与场景深度图直接相关）。在最初的工作中，通过采用昂贵的alpha matting策略，改进了传输。