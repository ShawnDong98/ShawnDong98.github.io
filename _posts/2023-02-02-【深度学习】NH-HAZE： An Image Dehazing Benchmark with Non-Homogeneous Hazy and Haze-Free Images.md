...
**Berman et al.** 的方法基于Omer等人引入的颜色一致性观察。这种方法认为，无雾图像中的颜色分布由 RGB 色彩空间中的一组离散簇很好地近似。基本上，这种方法假设给定簇中的像素是非局部的，并分布在整个图像平面上。因此，假设模糊区域的像素受到不同的影响。对于朦胧的图像，这些颜色簇在RGB色彩空间中成为不同的线条，称为雾霾线。像素在线中的位置反映了其传输等级。