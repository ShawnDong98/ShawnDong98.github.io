...
为了便于讨论，让我们假设梯度为160 MB。在这种情况下，将所有3个剩余 GPU 的梯度发送到第4个需要30毫秒(每次传输花费 10ms = 160MB / 16GB/s)。 再加上30毫秒来传输权重向量，我们得到了总共60毫秒。如果我们将所有数据发送到CPU， 这将导致40ms的惩罚， 因为四个GPU中的每一个都需要将数据发送到CPU， 总共产生80ms的乘法。